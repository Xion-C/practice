...
//
Quaternion::operator class Vector3d(){
  Vector3d vect;
  vect.x = q.x;
  vect.y = q.y;
  vect.z = q.z;
...
Quaternion Quaternion::normalize(){
  ...
  if (abs(q.x) > magnitude * HUGENUMBER ||
    ...
    cerr << "Attempting to take the norm of a zero quaternion." << endl;
  }
...
void Quaternion::GLrotate(){		// do the OpenGL rotation
  ...
  // avoid doing the rotation if the angle is nearly 0, since the
  // rotation axis will be suspect and can break OpenGL
  if(Abs(theta) > 0.001)
