...
Vector2d::Vector2d(double vx, double vy) { set(vx, vy); }

Vector2d::Vector2d(const Vector2d& v) { set(v); }

Vector3d::Vector3d(double vx, double vy, double vz) { set(vx, vy, vz); }

Vector3d::Vector3d(const Vector3d& v) { set(v); }

Vector4d::Vector4d(double vx, double vy, double vz, double vw) {
    set(vx, vy, vz, vw);
}

Vector4d::Vector4d(const Vector4d& v) { set(v); }
...
Vector::Vector(int vN, double* vx) {
    ...
    setsize(vN);
    if (vx != NULL)
        for (i = 0; i < N; i++) v[i] = vx[i];
}

Vector::Vector(const Vector& V) { set(V); }
...
void Vector::setsize(int vN) {
    ...
    if (N == 0)
        ...
        v = new double[N];
        if (!v) {
            cerr << "not enough memory to allocate vector of size " << N
                 << endl;
            exit(1);
        }
    }
...
Vector2d Vector2d::normalize() const {
    ...
    if (abs(x) > magnitude * HUGENUMBER || abs(y) > magnitude * HUGENUMBER) {
...
Vector4d Vector4d::wnorm() const {
    ...
    if (w == 1.0)
        ...
        cerr << "w-Normalizing vector [" << x << " " << y << " " << z << " "
             << w << "] w ~= 0!" << endl;
...
void Vector2d::print() const { cout << "[" << x << " " << y << "]"; }
...
double Vector2d::norm() const { return sqrt(normsqr()); }
double Vector3d::norm() const { return sqrt(normsqr()); }
double Vector4d::norm() const { return sqrt(normsqr()); }
double Vector::norm() const { return sqrt(normsqr()); }

// Compute the squared magnitude of a vector.
double Vector2d::normsqr() const { return Sqr(x) + Sqr(y); }
double Vector3d::normsqr() const { return Sqr(x) + Sqr(y) + Sqr(z); }
double Vector4d::normsqr() const { return Sqr(x) + Sqr(y) + Sqr(z) + Sqr(w); }
double Vector::normsqr() const {
    int i;
    double sumsqr = 0;
    for (i = 0; i < N; i++) sumsqr += Sqr(v[i]);
...
Vector operator+(const Vector& v1, const Vector& v2) {
    ...
    for (i = 0; i < v1.N; i++) r[i] = v1.v[i] + v2.v[i];
    return r;
}
...
double operator*(const Vector& v1, const Vector& v2) {
    if (v1.N != v2.N) {
        cerr << "cannot take dot product of " << v1.N << " Vector with " << v2.N
             << " Vector" << endl;
        exit(1);
    }
    ...
    for (i = 0; i < v1.N; i++) p += v1.v[i] * v2.v[i];
    return p;
}
