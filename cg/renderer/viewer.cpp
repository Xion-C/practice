...
void frame_display(color_t frame[HEIGHT][WIDTH]) {
    for (int i = 0; i < HEIGHT; i++) {
        for (int j = 0; j < WIDTH; j++) {
            write_pixel(j - biasx, i - biasy, frame[i][j]);
...
color_t fragment_shader(const light_t& light, const point_t& camera, const primitive_t& f, point_t p) {
    ...
    N = f.n;
    //N.x = (f.vn1.x + f.vn1.x + f.vn3.x)/3;
    //N.y = (f.vn1.y + f.vn2.y + f.vn3.y)/3;
    //N.z = (f.vn1.z + f.vn3.z + f.vn3.z)/3;
    ...
    if (round(p.x) == click.x && round(p.y) == click.y && clickflag) {
        printf("©°©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤ shader print start ©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©´\n");
        //primitive_printInfo(f);
        float xtrans = float(WIDTH) / float(objdata.texw);
        float ytrans = float(HEIGHT) / float(objdata.texh);
        click.xtex = round(xtex * xtrans) - biasx + WIDTH;
        click.ytex = HEIGHT - round(ytex * ytrans) - biasy;
        primitive_printInfo(f);
        printf(" *** kd: %f\n", kd);
        printf("©¸©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤  shader print end  ©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¼\n");
    }
...
void primitive_perspective(primitive_t& f) {
    if (showperspective == 1) {
        matrix_t p;
        matrix_set_perspective(p, objdata.center.z + objdata.bounding.z / 2, render.camera);
...
void primitive_set_plane_equation(primitive_t& f_t) {
    float x1 = f_t.v1.x;
    float x2 = f_t.v2.x;
    float x3 = f_t.v3.x;
    float y1 = f_t.v1.y;
...
void primitive_rasterization(const primitive_t& f) {
    ...
    float accuracy12 = 0.05;
    float accuracy23 = 0.05;
    float accuracy31 = 0.05;
    ...
    float z = 0;
    primitive_set_plane_equation(f_t);
        ...
        for (int x = minx; x < maxx; x++) {
            ...
            //    (cx1 <= 0 && cx2 <= 0 && cx3 <= 0)) {
                if (!f_t.zn) { //when f.n.zn is false, means it's a line
                    ...
                    float t = ((dx1p / dx12) + (dy1p / dy12)) / 2;
                    float z1 = f_t.v1.z - t * (f_t.v1.z - f_t.v2.z);
                    t = ((dx1p / -dx31) + (dy1p / -dy31)) / 2;
                    float z2 = f_t.v1.z - t * (f_t.v1.z - f_t.v3.z);
                    z = z1 > z2 ? z1 : z2;
                }
                else {
                    //plane
                ...
                if (x == click.x && y == click.y && clickflag) {
                    primitive_printInfo(f);
                    printf("z pers: %f, z origin: %f\n",
                        z, (-f.d - f.n.x * x0 - f.n.y * y0) / f.n.z);
                }
...
void vertices_init() {
    ...
    objdata.bounding.x = v.x * rscale;
    objdata.bounding.y = v.y * rscale;
    objdata.bounding.z = v.z * rscale;
...
void primitives_init() {
    ...
    for (unsigned int i = 0; i < (objdata.vertex_indices.size()); i++) {
        ...
        primitive_set_plane_equation(face);
...
void view_obj() {
    ...
    //primitive_t f;
    //int fn = objdata.faces.size();
    //static int cfn = 0;
    //if(cfn == 0)  render_init();
    //if (showframe == 0) {
    //    //primitive_printInfo(objdata.faces[i]);

    //    primitive_rasterization(objdata.faces[cfn]);
    //}
    //else {
    //    primitive_t f = objdata.faces[cfn];
    //    primitive_perspective(f);
    //    draw_line(f.v1, f.v2, white);
    //    draw_line(f.v2, f.v3, white);
    //    draw_line(f.v1, f.v3, white);
    //}
    //cfn = cfn >= fn - 1 ? 0 : cfn + 1;
    ...
    frame_display(render.frame);
    textureMap_display();


    if (clickflag) {
        printf("zbuf: %f, zbuf_last: %f\n", render.zbuffer[click.y + biasy][click.x + biasx], render.zbuffer_last[click.y + biasy][click.x + biasx]);
        printf("__________________________________________________________\n");
        clickflag = 0;
    }

    click_display(click.x, click.y);
    click_display(click.xtex, click.ytex);
            ...
            if (render.frame[i][j].r > 0.00001f || render.frame[i][j].g > 0.00001f|| render.frame[i][j].b > 0.00001f) {
                for (int m = -b; m <= b; m++) {
                    for (int n = -b; n <= b; n++) {
                        ...
                        fbuffer.f1[i][j].r = saturate(1.001 * fbuffer.f1[i][j].r,0.0f, 1.0f);
                        fbuffer.f1[i][j].g = saturate(1.0 * fbuffer.f1[i][j].g, 0.0f, 1.0f);
                        fbuffer.f1[i][j].b = saturate(0.999 * fbuffer.f1[i][j].b, 0.0f, 1.0f);
...
void gaussian6(int ksize) {
    ...
    //printf("gaussian 6 done\n");

}

void textureMap_display() {
    float xscale = float(objdata.texw - 1) / float(WIDTH - 1);
    float yscale = float(objdata.texh - 1) / float(HEIGHT - 1);
    for (int i = 0; i < HEIGHT; i++) {
        for (int j = 0; j < WIDTH; j++) {
            ...
            float x = xi - xl;
            float y = yi - yl;
            color_t a00 = objdata.texture[yl][xl];
            color_t a01 = color_minus(objdata.texture[yh][xl], objdata.texture[yl][xl]);
            color_t a10 = color_minus(objdata.texture[yl][xh], objdata.texture[yl][xl]);
            color_t a11 = color_minus(
                color_plus(objdata.texture[yh][xh], objdata.texture[yl][xl]),
                color_plus(objdata.texture[yh][xl], objdata.texture[yl][xh])
            );
            color_t res = color_plus(
                color_plus(a00, color_scalar(x, a10)),
                color_plus(color_scalar(y, a01), color_scalar(x*y, a11))
            );
            write_pixel(j + WIDTH - biasx, HEIGHT - i - biasy, res);
        }
    }
}
...
void click_display(int x, int y) {
    int cross_size = 3;
    for (int i = x - cross_size; i <= x + cross_size; i++) {
        write_pixel(i,y, red);
    }
    for (int i = y - cross_size; i <= y + cross_size; i++) {
        write_pixel(x, i, red);
    }
}