...
void write_pixel(int x, int y, float intensity[3])
...
void draw_control_point(float intensity[3])
/* Draw the area around control point that could be clicked */
{
	...
	for(i = 0; i <= (init_p ? chosen_p : P_NUM - 1); i++) {
		intensity_c = (chosen_p == i && n_frames == 0) ? 0.3 : 1.0; //the chosen point area will have lower intensity
		for(int rgb = 0; rgb < 3; rgb++) color_temp[rgb] = intensity_c * intensity[rgb];
...
void display ( void )   // Create The Display Function
{
	...
	set_control_point();
	draw_control_point(color_white);
	if(!init_p) {
