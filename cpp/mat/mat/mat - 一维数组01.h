...
public:
    ...
    void print();
    T* ptr() { return mat; }
    T& at(const int& h, const int& w) { return mat[h * _col + w]; }
    //Mat<T> transpose();
    //Mat<T> submat(int rowStart, int rowEnd, int colStart, int colEnd);

    //read only member functions
    const bool isEmpty() const;
    const T& at(const int& h, const int& w) const { return mat[h * _col + w]; }
    const int row() const { return _row; }
    const int col() const { return _col; }

    //operator overload
    T* const operator[](const int i) { return &mat[i * _col]; }
...
void Mat<T>::load(const char* path, int height, int width) {
    if (mat && !(height == _row && width == _col)) {
        ...
        fp = fopen(path, "rb");
        mat = new T[height * width];
        fread(mat, sizeof(T), height * width, fp);
        fclose(fp);
